...
import createStyles from '../../styles/client/ClienteResenasStyles';
import { resenaAPI, tokenManager, clienteAPI } from '../../config/api';
...
const ClienteResenasScreen = ({ navigation }) => {
  ...
  const cargarDatos = useCallback(async () => {
    ...
    const resBarberia = await clienteAPI.obtenerBarberiaAsociada(user.id);
    if (!resBarberia.success) {
      setError(resBarberia.error || 'No se pudo obtener tu barbería asociada');
      setCargando(false);
      return;
    }

    const idBarberiaActual = resBarberia.data.id;
        ...
        <View style={styles.contentWrap}>
          ...
          {reseñas.length === 0 && !cargando ? (
            ...
            <View style={styles.reviewsGrid}>
              {(mostrarTodas ? reseñas : reseñas.slice(0, 5)).map((reseña) => (
...
export default ClienteResenasScreen;