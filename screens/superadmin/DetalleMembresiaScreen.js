...
const DetalleMembresiaScreen = ({ navigation, route }) => {
  ...
  const adeudo =
    Math.round(
      pendientes.reduce(
        (acc, p) =>
          acc + (p.monto?.parsedValue ?? Number(p.monto?.source ?? 0)),
        0,
      ) * 100,
    ) / 100;

  const montoMensual =
    pagados[0]?.monto?.parsedValue ?? pagados[0]?.monto?.source ?? null;
  ...
  const handleCrearMembresia = async () => {
    ...
    const res = await suscripcionAPI.crear(barberiaId);
    ...
    setResultado({
      ...
      title: "¡Listo!",
      message:
        "La membresía se creó correctamente, con su primer pago pendiente.",
    });
    cargarDatos();
  };
  ...
  const estadoInfo = (() => {
    if (!suscripcion)
      return { color: "#EF4444", label: "Sin membresía activa" };
    if (suscripcion.estado === "ACTIVA")
      return { color: "#22C55E", label: "Membresia activa" };
    if (suscripcion.estado === "SUSPENDIDA")
      return { color: "#F59E0B", label: "Membresia suspendida" };
  ...
  return (
    ...
    >
        ...
        <View style={styles.contentWrap}>
          ...
          <View style={styles.tabsRow}>
            {TABS.map((tab) => (
              <TouchableOpacity
                ...
                style={[
                  styles.tabPill,
                  tabActivo === tab && styles.tabPillActive,
                ]}
          ...
          {tabActivo === "Membresia" ? (
            !suscripcion ? (
              ...
              <View style={styles.card}>
                <Text style={styles.barberiaNombre}>
                  {barberia?.nombre || "—"}
                </Text>
                ...
                <Text style={styles.value}>
                  {formatearFechaLarga(suscripcion?.fechaInicio)}
                </Text>

                <Text style={styles.label}>Vencimiento</Text>
                <Text style={styles.value}>
                  {formatearFechaLarga(suscripcion?.fechaFin)}
                </Text>

                <Text style={styles.label}>Pago mensual</Text>
                <Text style={styles.value}>
                  {montoMensual
                    ? `$${montoMensual} MXN / mes`
                    : "$499 MXN / mes"}
                </Text>
                        ...
                        {pagos
                          ...
                          .map((p) => (
                            <View key={p.id} style={styles.pagoItem}>
                              <Text style={styles.pagoItemFecha}>
                                {formatearMesAnio(p)}
                              </Text>
                              <View style={styles.pagoItemCentro}>
                                ...
                                >
                                  <Text style={styles.pagoBtnRegistrarText}>
                                    Registrar pago
                                  </Text>
                                </TouchableOpacity>
                              </View>
                    ...
                    {pagos
                      ...
                      .map((p) => (
                        <View key={p.id} style={styles.pagoItem}>
                          ...
                          <View style={styles.pagoItemCentro}>
                            <Text style={styles.pagoItemMonto}>
      ...
      <LoadingOverlay
        visible={creandoSuscripcion}
        message="Creando membresía..."
      />
          ...
          <View style={styles.modalCard}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity
                ...
                onPress={() => setPagoARegistrar(null)}
              >
                <Ionicons
                  name="close"
                  size={18}
            ...
            >
              {["EFECTIVO", "TRANSFERENCIA"].map((metodo) => (
                <TouchableOpacity
                  key={metodo}
                  onPress={() =>
                    setFormPago((prev) => ({ ...prev, metodoPago: metodo }))
                  }
                  style={{
            ...
            <TextInput
              ...
              onChangeText={(v) =>
                setFormPago((prev) => ({ ...prev, referencia: v }))
              }
              style={{
                color: theme.mode === "dark" ? "#FFFFFF" : "#1A1A1A",
                borderWidth: 1,
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity
                ...
                onPress={confirmarRegistrarPago}
              >
