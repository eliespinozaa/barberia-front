...
const BarberHomeScreen = ({ navigation }) => {
  ...
  const cargarDatos = useCallback(async () => {
    ...
    const resBarberia = await barberiaAPI.obtenerPorId(barbero.idBarberia);
    if (resBarberia?.success) {
      setBarberiaInfo(resBarberia.data);
    }
  ...
  const STATS = [
    {
      label: "Citas de hoy",
      value: String(statsData.hoy),
      icon: "calendar-outline",
    },
    {
      label: "Completadas",
      value: String(statsData.completadas),
      icon: "checkmark-circle-outline",
    },
    {
      label: "Canceladas",
      value: String(statsData.canceladas),
      ...
      <View style={styles.navbar}>
        <View style={styles.navLeft}>
          ...
          <View style={styles.navLogoWrap}>
            {barberiaInfo?.imagen ? (
              <Image
                source={{ uri: construirUriImagen(barberiaInfo.imagen) }}
      ...
      <View style={styles.bodyRow}>
        ...
        >
          ...
          {citasHoy.length === 0 && !cargando ? (
            ...
            citasHoy.map((cita) => {
              const estado =
                ESTADO_LABELS[cita.estado] ?? ESTADO_LABELS.PENDIENTE;
              return (
                <TouchableOpacity key={cita.id} style={styles.citaCard}>
                  <View style={styles.citaRow}>
                    ...
                    <View style={styles.citaMeta}>
                      ...
                      <View
                        style={[
                          styles.citaEstadoBadge,
                          { borderColor: estado.color },
                        ]}
                      >
                        <Text
                          style={[
                            styles.citaEstadoText,
                            { color: estado.color },
                          ]}
                        >
                          {estado.label}
